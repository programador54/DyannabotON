...
module.exports = {
	...
	async execute(client, message, args) {
	  await message.delete()
		...
		const fundo1 = await loadImage(
			'https://cdn.discordapp.com/attachments/688436947516915764/701160106296868955/perfeito.png'
		);
		ctx.drawImage(fundo1, 0, 0, canvas.width, canvas.height);

		//carrega 1 img, que no caso eo avatar do membro
		const avatar = await loadImage(
			usuario.avatarURL({ format: 'png', dynamic: true, size: 1024 })
		); //carrega o avatarURL
		ctx.drawImage(avatar, height - 177, posX + 20, height - 178, height - 179); //posiciona o avatar

		//carrega 2 img
		const image2 = await loadImage(
			'https://cdn.discordapp.com/attachments/688436947516915764/701160545440497735/redondo.png?width=475&height=475'
		);
